...
// link, not the target: a page that exists only as a draft is invisible on the
// published perspective this build fetches with, so it dereferences to null and
// would otherwise be projected as `{title: null, slug: null}` — which Header and
// Footer interpolate straight into an href, shipping `<a href="/null">` with no
// text: invisible, clickable, and a 404.
