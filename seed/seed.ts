...
async function main() {
  ...
  const existingLegalPage = await client.fetch(`*[_type == "legalPage"][0]._id`)
  if (existingLegalPage) {
    console.log(`skipped legalPage docs — already present (found ${existingLegalPage})`)
  } else {
    await client.create({
      _type: 'legalPage',
      title: 'imprint',
